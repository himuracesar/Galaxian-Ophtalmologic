...
DOGRESULT Nivel5::Init()
{
	cout << "init Nivel 5" << endl;
...
DOGvoid Nivel5::Update(DOGfloat elapsedTime)
{
	if (play)
	{
		//Actualiza el player
...
DOGvoid Nivel5::Input(DOG_INPUT_MOUSE dim, DOGBOOL* pPressedKeys, vector<DOGBOOL>* pLockKeys, DOGfloat elapsedTime)
{
...
DOGvoid Nivel5::IniciaTropa(DOGbool f)
{
	...
	for (auto row = 0; row < nyEnemies; row++)
	{
		...
		for (auto inx = 0; inx < nxEnemies; inx++)
		{
			...
			if (enemy == "E" || !f)
			{
				...
				{
				case 1:
					e = new Enemy("glx/img/5/1.png", 42, 38);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 2:
					e = new Enemy("glx/img/5/2.png", 45, 38);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 3:
					e = new Enemy("glx/img/5/3.png", 38, 38);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 4:
					e = new Enemy("glx/img/5/4.png", 37, 38);
					//e->CreateBoundingBox();
					break;
				}
...
DOGvoid Nivel5::DeleteEnemy(DOGint id)
{
	for (auto iEnemy = 0; iEnemy < pEnemigos->size(); iEnemy++)
	{
...
DOGvoid Nivel5::Explote(DOGfloat x, DOGfloat y, DOGfloat z)
{
	DOGbool free = false;
	for (auto iExplosion = 0; iExplosion < pExplosions->size(); iExplosion++)
	{
...
DOGvoid Nivel5::CreateExplosion()
{
...
DOGbool Nivel5::CheckExplosionLive()
{
