...
DOGRESULT Nivel6::Init()
{
	cout << "init Nivel 6" << endl;
...
DOGvoid Nivel6::Update(DOGfloat elapsedTime)
{
	if (play)
	{
		//Actualiza el player
...
DOGvoid Nivel6::Input(DOG_INPUT_MOUSE dim, DOGBOOL* pPressedKeys, vector<DOGBOOL>* pLockKeys, DOGfloat elapsedTime)
{
...
DOGvoid Nivel6::IniciaTropa(DOGbool f)
{
	...
	for (auto row = 0; row < nyEnemies; row++)
	{
		...
		for (auto inx = 0; inx < nxEnemies; inx++)
		{
			...
			if (enemy == "E" || !f)
			{
				...
				{
				case 1:
					e = new Enemy("glx/img/6/1.png", 29, 26);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 2:
					e = new Enemy("glx/img/6/2.png", 31, 26);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 3:
					e = new Enemy("glx/img/6/3.png", 27, 26);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 4:
					e = new Enemy("glx/img/6/4.png", 26, 26);
					//e->CreateBoundingBox();
					break;
				}
...
DOGvoid Nivel6::DeleteEnemy(DOGint id)
{
	for (auto iEnemy = 0; iEnemy < pEnemigos->size(); iEnemy++)
	{
...
DOGvoid Nivel6::Explote(DOGfloat x, DOGfloat y, DOGfloat z)
{
	DOGbool free = false;
	for (auto iExplosion = 0; iExplosion < pExplosions->size(); iExplosion++)
	{
...
DOGvoid Nivel6::CreateExplosion()
{
...
DOGbool Nivel6::CheckExplosionLive()
{
