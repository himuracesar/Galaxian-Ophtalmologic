...
DOGRESULT Nivel2::Init()
{
	cout << "init Nivel 2" << endl;
...
DOGvoid Nivel2::Update(DOGfloat elapsedTime)
{
	if (play)
	{
		//Actualiza el player
...
DOGvoid Nivel2::Input(DOG_INPUT_MOUSE dim, DOGBOOL* pPressedKeys, vector<DOGBOOL>* pLockKeys, DOGfloat elapsedTime)
{
...
DOGvoid Nivel2::IniciaTropa(DOGbool f)
{
	...
	for (auto row = 0; row < nyEnemies; row++)
	{
		...
		for (auto inx = 0; inx < nxEnemies; inx++)
		{
			...
			if (enemy == "E" || !f)
			{
				...
				{
				case 1:
					e = new Enemy("glx/img/1/1.png", 101, 91);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 2:
					e = new Enemy("glx/img/1/2.png", 107, 83);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 3:
					e = new Enemy("glx/img/1/3.png", 96, 94);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 4:
					e = new Enemy("glx/img/1/4.png", 92, 94);
					//e->CreateBoundingBox();
					break;
				}
...
DOGvoid Nivel2::DeleteEnemy(DOGint id)
{
	for (auto iEnemy = 0; iEnemy < pEnemigos->size(); iEnemy++)
	{
...
DOGvoid Nivel2::Explote(DOGfloat x, DOGfloat y, DOGfloat z)
{
	DOGbool free = false;
	for (auto iExplosion = 0; iExplosion < pExplosions->size(); iExplosion++)
	{
...
DOGvoid Nivel2::CreateExplosion()
{
...
DOGbool Nivel2::CheckExplosionLive()
{
