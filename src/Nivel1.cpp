...
DOGRESULT Nivel1::Init()
{
	cout << "init Nivel 1" << endl;
	...
	//pPlayer->SetColor(DOGCOLOR(1.0f, 1.0f, 1.0f, 0.3f));
...
DOGvoid Nivel1::Update(DOGfloat elapsedTime)
{
	if (play)
	{
		//Actualiza el player
		...
		for (auto iEnemy = 0; iEnemy < pEnemigos->size(); iEnemy++)
		{
			...
			if ((*pEnemigos)[iEnemy]->IsFire())
			{
				...
				Sprite* ps = new Sprite("glx/img/bullet8x8.png", 16, 16);
...
DOGvoid Nivel1::Input(DOG_INPUT_MOUSE dim, DOGBOOL* pPressedKeys, vector<DOGBOOL>* pLockKeys, DOGfloat elapsedTime)
{
...
DOGvoid Nivel1::IniciaTropa()
{
	...
	for (auto row = 0; row < nyEnemies; row++)
	{
		for (auto inx = 0; inx < nxEnemies; inx++)
		{
			...
			Enemy* e = new Enemy("glx/img/1/C.png", 112, 112);
			//switch (n)
			//{
			//	case 1:
			//		e = new Enemy("glx/img/1/1.png", 109, 93);
			//		//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
			//	break;
			//	case 2:
			//		e = new Enemy("glx/img/1/2.png", 110, 93);
			//		//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
			//	break;
			//	case 3:
			//		e = new Enemy("glx/img/1/3.png", 110, 93);
			//		//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
			//	break;
			//	case 4:
			//		e = new Enemy("glx/img/1/4.png", 107, 110);
			//		//e->CreateBoundingBox();
			//	break;
			//}
...
DOGvoid Nivel1::DeleteEnemy(DOGint id)
{
	for (auto iEnemy = 0; iEnemy < pEnemigos->size(); iEnemy++)
	{
...
DOGvoid Nivel1::Explote(DOGfloat x, DOGfloat y, DOGfloat z)
{
	DOGbool free = false;
	for (auto iExplosion = 0; iExplosion < pExplosions->size(); iExplosion++)
	{
...
DOGvoid Nivel1::CreateExplosion()
{
...
DOGbool Nivel1::CheckExplosionLive()
{
