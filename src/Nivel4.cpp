...
DOGvoid Nivel4::Update(DOGfloat elapsedTime)
{
	if (play)
	{
		//Actualiza el player
...
DOGvoid Nivel4::Input(DOG_INPUT_MOUSE dim, DOGBOOL* pPressedKeys, vector<DOGBOOL>* pLockKeys, DOGfloat elapsedTime)
{
...
DOGvoid Nivel4::IniciaTropa(DOGbool f)
{
	...
	for (auto row = 0; row < nyEnemies; row++)
	{
		...
		for (auto inx = 0; inx < nxEnemies; inx++)
		{
			...
			if (enemy == "E" || !f)
			{
				...
				{
				case 1:
					e = new Enemy("glx/img/1/1.png", 76, 68);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 2:
					e = new Enemy("glx/img/1/2.png", 74, 64);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 3:
					e = new Enemy("glx/img/1/3.png", 69, 68);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 4:
					e = new Enemy("glx/img/1/4.png", 66, 68);
					//e->CreateBoundingBox();
					break;
				}
...
DOGvoid Nivel4::DeleteEnemy(DOGint id)
{
	for (auto iEnemy = 0; iEnemy < pEnemigos->size(); iEnemy++)
	{
...
DOGvoid Nivel4::Explote(DOGfloat x, DOGfloat y, DOGfloat z)
{
	DOGbool free = false;
	for (auto iExplosion = 0; iExplosion < pExplosions->size(); iExplosion++)
	{
...
DOGvoid Nivel4::CreateExplosion()
{
...
DOGbool Nivel4::CheckExplosionLive()
{
