...
DOGRESULT Nivel3::Init()
{
	cout << "init Nivel 3" << endl;
...
DOGvoid Nivel3::Update(DOGfloat elapsedTime)
{
	if (play)
	{
		//Actualiza el player
...
DOGvoid Nivel3::Input(DOG_INPUT_MOUSE dim, DOGBOOL* pPressedKeys, vector<DOGBOOL>* pLockKeys, DOGfloat elapsedTime)
{
...
DOGvoid Nivel3::IniciaTropa(DOGbool f)
{
	...
	for (auto row = 0; row < nyEnemies; row++)
	{
		...
		for (auto inx = 0; inx < nxEnemies; inx++)
		{
			...
			if (enemy == "E" || !f)
			{
				...
				{
				case 1:
					e = new Enemy("glx/img/1/1.png", 88, 79);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 2:
					e = new Enemy("glx/img/1/2.png", 94, 79);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 3:
					e = new Enemy("glx/img/1/3.png", 80, 79);
					//e->CreateBoundingCircle(e->GetWidth()/2.0f, x, y);
					break;
				case 4:
					e = new Enemy("glx/img/1/4.png", 77, 79);
					//e->CreateBoundingBox();
					break;
				}
...
DOGvoid Nivel3::DeleteEnemy(DOGint id)
{
	for (auto iEnemy = 0; iEnemy < pEnemigos->size(); iEnemy++)
	{
...
DOGvoid Nivel3::Explote(DOGfloat x, DOGfloat y, DOGfloat z)
{
	DOGbool free = false;
	for (auto iExplosion = 0; iExplosion < pExplosions->size(); iExplosion++)
	{
...
DOGvoid Nivel3::CreateExplosion()
{
...
DOGbool Nivel3::CheckExplosionLive()
{
